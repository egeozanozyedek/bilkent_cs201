...
bool FC::addFilm(const string fTitle, const string fDirector, const unsigned int fYear, const unsigned int fDuration) {
    ...
    for (int i = 0; i < noOfFilms; i++) {
        if (films[i].getFilmTitle() == fTitle and films[i].getFilmDirector() == fDirector)
            return false;
    }
...
/*
 ...
 If there are no films returns false
 Goes through all Film objects in films and calls its member function removeActors which returns boolean value
 If no film with given info is found returns false
 */
...
/*
addActor function
Checks whether given film exists in array
 If it does, calls the addActor function of the Film object and returns its return value
 If no matching film is found returns false
*/
...
/*
 calculateAverageDuration function
 Simpe arithmetic function whic uses getFilmDuration and finds the average film duration in the film collection
 */
